...
if (!class_exists('WPBakeryVisualComposer')) {
    class WPBakeryVisualComposer extends WPBakeryVisualComposerAbstract {
       ...
       ---------------------------------------------------------- */
        ...
        public function shortCodesVisualComposerJavascript_callback() {
            ...
            foreach($not_shortcodes as $string ) {
                if( strlen(trim($string))>0 ) {
                    $content = preg_replace("/(".preg_quote($string, '/')."(?!\[\/))/", '[vc_row][vc_column width="1/1"][md_text width="1/1" el_position="first last"]$1[/md_text][/vc_column][/vc_row]', $content);
                }
            }
        ...
        public function reloadTemplatesJavascript_callback() {
