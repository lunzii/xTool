...
(function ($) {
    ...
    var SettingsView = Backbone.View.extend({
        ...
        initDependency:function () {
            ...
            $( ".slider-input" ).each(function(){
                $slider_value = $(this).data('value');
                $slider_min = $(this).data('min');
                $slider_max = $(this).data('max');
                ...
                $(this).slider({
                    ...
                    slide: function( event, ui ) {
                       $(this).next('.slider-amount').val( ui.value );
                    }
                });
                $(this).next('.slider-amount').val( $(this).slider('value') );


            });

            $('.slider-amount').change(function(){
                $val = $(this).val();
                $(this).prev('.slider-input').slider('value', $val);
            });
        },
    ...
    var VisualComposer = Backbone.View.extend({
        ...
        events:{
            ...
            'click .wpb_import_templates':'importDemoTemplates'
        },
        initialize:function () {
        ...
        importDemoTemplates:function(e){
            

            var confirmation = confirm("Load demo templates?")
            if (confirmation){
                if($('.wpb_import_templates').hasClass('disabled')) return false;
                $('.wpb_import_templates').addClass('disabled');
                ...
                $.ajax({
                    ...
                    $.ajax({
                        ...
                        url:window.ajaxurl,
                        data:{
                            action:'wpb_reload_templates',
                        },
                        context:this
                    }).done(function (html) {
                       $('.wpb_templates_list').html(html);
                       alert('Demo templates loaded');
                    });
        ...
        addTextBlock:function (e) {
            ...
            var row = Shortcodes.create({shortcode:'vc_row'}),
                column = Shortcodes.create({shortcode:'vc_column', params:{width:'1/1'}, parent_id:row.id, root_id:row.id }),
                text_block = Shortcodes.create({shortcode:'md_text', params:vc.getDefaults('md_text'), parent_id:column.id, root_id:row.id });
            return text_block;
        },
        /**
         * Create row
        ...
        createRow:function () {
            ...
            offsetLastRow = $('.wpb_vc_row:last').offset().top - 100;
            $('body').animate({ scrollTop : offsetLastRow}, function(){
                $('.wpb_vc_row:last .column_edit').eq(0).click();
            });
        ...
        addElement:function (e) {
            ...
            if($('.wpb_vc_row:last').length){
                offsetLastRow = $('.wpb_vc_row:last').offset().top - 100;
                $('body, html').animate({ scrollTop : offsetLastRow});
            }
        },
