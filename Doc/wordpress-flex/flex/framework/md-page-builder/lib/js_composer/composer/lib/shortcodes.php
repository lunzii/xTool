...
if (!class_exists('WPBakeryShortCode')) {
    ...
    ---------------------------------------------------------- */
        public function getColumnControls($controls, $extended_css = '') {
            ...
            $controls_edit = ' <a class="column_edit" href="#" title="'.sprintf(__('Edit %s', 'js_composer'), strtolower($this->settings('name'))).'"><i class="icon-pencil"></i></a>';
            $controls_delete = ' <a class="column_clone" href="#" title="'.sprintf(__('Clone %s', 'js_composer'), strtolower($this->settings('name'))).'"><i class="icon-copy"></i></a> <a class="column_delete" href="#" title="'.sprintf(__('Delete %s', 'js_composer'), strtolower($this->settings('name'))).'"><i class="icon-trash"></i></a>';
...
if (!class_exists('WPBakeryShortCode_UniversalAdmin')) {
    abstract class WPBakeryShortCode_UniversalAdmin extends WPBakeryShortCode {
        ...
        protected function singleParamEditHolder($param, $param_value) {
            $output = '<div class="vc_row-fluid wpb_el_type_'. $param['type'] .' md-field">';
            $output .= (isset($param['heading'])) ? '<div class="wpb_element_label label">'.__($param['heading'], "js_composer").'</div>' : '';//vc_span3
        ...
        protected function singleParamEditForm($param, $param_value) {
            ...
            // Dropdown - select
            else if ( $param['type'] == 'dropdown' || $param['type'] == 'radio' ) {
            ...
            // Dropdown Multiple
            else if ( $param['type'] == 'dropdown_multiple' ) {
            ...
            // Textarea Shortcode 
            else if ( $param['type'] == 'textarea_shortcode' ) {
            ...
            // Slider 
                ...
                if(!$value) $value = intval($param['default']);
